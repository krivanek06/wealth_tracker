...
export class CreateAssetStockUtil {
	static createAssetStock = (quote: FMQuote, profile: FMProfile): AssetStock => {
		const assetStockProfile = this.convertFMProfileToAssetStockProfile(profile);
		const assetStockQuote = this.convertFMQuoteToAssetStock(quote);

		const result: AssetStock = {
			symbol: quote.symbol,
			infoUpdateTimestamp: quote.timestamp,
			priceUpdateTimestamp: quote.timestamp,
			assetStockProfile,
			assetStockQuote,
		};
	...
	static convertFMQuoteToAssetStock = (quote: FMQuote): AssetStockQuote => {
